...
function advanceBases(bases: boolean[], hitType: string): { newBases: boolean[], runsScored: number } {
  ...
  if (hitType === "2B") {
    runs = (b[2]?1:0) + (b[1]?1:0);
    return { newBases: [false,true,b[0]], runsScored: runs };
  }
  // 1B or E: 3rd scores, others advance 1
  runs = b[2] ? 1 : 0;
  return { newBases: [true,b[0],b[1]], runsScored: runs };
}

function walkBases(bases: boolean[]): { newBases: boolean[], runsScored: number } {
  const b = [...bases]; let runs = 0;
  if (b[0]) { if (b[1]) { if (b[2]) runs = 1; b[2] = true; } b[1] = true; }
  b[0] = true;
...
export function LiveGame({ data, id, nav }: any) {
  ...
  const [showConfirm, setShowConfirm] = useState<any>(null); // post-play confirmation
  ...
  const getPlayerGameStats = (playerId: string) => {
    let vb=0,h=0,hr=0,ci=0,ca=0,bb=0,k=0,db=0,tb=0,sb=0;
    plays.forEach((p: any) => {
      ...
      else if (["OUT","FLY","GROUND","K","DP","SAC"].includes(p.result)) { vb++; if (p.result==="K") k++; }
      else if (p.result==="E") { vb++; }
      ci += (p.ci||0); ca += (p.ca||0);
      if (p.result==="SB") sb++;
    });
    return { vb,h,hr,ci,ca,bb,k,db,tb,sb, summary: `${h}-${vb}${hr>0?`, ${hr}HR`:""}${ci>0?`, ${ci}CI`:""}` };
  };

  const getPitcherGameStats = (pitcherId: string) => {
    let h=0,bb=0,k=0,cl=0,outs=0,pitches=0;
    plays.forEach((p: any) => {
      if (p.pitcherId !== pitcherId) return;
      if (p.isPitch) pitches++;
      if (!p.result) return;
  ...
  if (game.status === "scheduled") {
    ...
    return (
      <div style={S.sec}>
        ...
        {[{ label: "Visitante", team: aw, players: awP, lineup: game.awayLineup||[], side: "away" as const },
          { label: "Local", team: hm, players: hmP, lineup: game.homeLineup||[], side: "home" as const }].map(({ label, team, players, lineup, side }) => (
          <div key={side} style={{ ...S.card, marginBottom: 12, overflow: "hidden" }}>
            ...
            <div style={{ padding: 10 }}>{players.map((p: any) => { const inL = lineup.find((l: any) => l.id === p.id); return (
              <div key={p.id} onClick={() => toggle(p.id, side)} style={{ display: "flex", alignItems: "center", padding: "8px 6px", borderBottom: `1px solid ${K.border}`, cursor: "pointer", background: inL?`${K.accent}11`:"transparent", borderRadius: 8, marginBottom: 2 }}>
                ...
                <span style={{ fontSize: 10, color: K.muted }}>{p.position}</span></div>); })}</div></div>))}
        <button onClick={async () => await up({ status: "live", count: { balls: 0, strikes: 0 }, awayBatterIdx: 0, homeBatterIdx: 0 })} style={{ ...S.btn("primary"), width: "100%", padding: 16, fontSize: 15 }}>
          <span style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}><IcoPlay size={18} />INICIAR JUEGO</span></button></div>
    );
  }
  ...
  const changeHalf = (updates: any) => {
    ...
    else { const hi = [...(game.homeInnings||[])]; if (hi[game.inning-1]===null) hi[game.inning-1]=0; updates.homeInnings = hi; updates.inning = game.inning+1; updates.half = "top"; }
  };

  const makePlay = (result: string, extra: any = {}) => ({
    playerId: currentBatter?.id, playerName: currentBatter?.name||"?",
    teamId: isTop ? game.awayTeamId : game.homeTeamId,
    team: isTop ? "away" : "home", result,
    ci: 0, ca: 0,
  ...
  // ── HITS: Show confirmation with adjustable runs ──
  ...
  const confirmHit = async () => {
    ...
    // CI = runs scored by OTHERS (not the batter). For HR solo = 1 run but CI=0 if bases empty
    // Actually in baseball: CI = all runs that score because of this at-bat, INCLUDING the batter on HR
    // But the batter's own run on HR is counted in the total runs, CI counts everyone
    const totalRuns = runs; // total runs that score on this play (includes batter on HR)
  ...
  const registerError = async (fielderPos: string) => {
    ...
    setShowError(false);
    setShowConfirm({
      type: "E", suggestedRuns: runsScored, runs: runsScored, newBases, ca: 0,
      extra: { errorPosition: fielderPos, isPitch: true },
    });
  };
  ...
  const registerComplex = async (type: string) => {
    if (type === "HBP") {
      const { newBases, runsScored } = walkBases(bases);
      const play = makePlay("HBP", { ci: runsScored, isPitch: true });
      await up({ plays: [...plays, play], bases: newBases, count: resetCount(), [batIdxKey]: nextBatter(), ...scoreRuns(runsScored) });
    } else if (type === "SB") {
  ...
  const undoLastPlay = async () => {
    ...
    if (!confirm("¿Deshacer última jugada?")) return;
    const removed = plays[plays.length - 1];
    const newPlays = plays.slice(0, -1);
    const u: any = { plays: newPlays };
    if (removed.result && removed.playerId) {
      const runs = (removed.ci||0);
      if (runs > 0) {
        const k = removed.team==="away"?"awayScore":"homeScore"; const ik = removed.team==="away"?"awayInnings":"homeInnings";
        u[k] = Math.max(0, (game[k]||0)-runs);
        const ni = [...(game[ik]||[])]; ni[removed.inning-1] = Math.max(0, (ni[removed.inning-1]||0)-runs); u[ik] = ni;
      }
      const bKey = removed.team==="away"?"awayBatterIdx":"homeBatterIdx";
      const ln = removed.team==="away"?(game.awayLineup||[]):(game.homeLineup||[]);
      const cur = game[bKey]||0;
      u[bKey] = cur > 0 ? cur-1 : ln.length-1;
    }
    await up(u);
  };
  ...
  const BtnAction = ({ label, icon, color, bg, onClick, size = "md" }: any) => (
    <button onClick={onClick} style={{
      padding: size==="lg"?"14px 8px":size==="sm"?"8px 6px":"10px 8px",
      borderRadius: 12, border: `2px solid ${color}44`, background: bg||`${color}15`,
      color, fontWeight: 900, fontSize: size==="lg"?13:size==="sm"?10:11,
      cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 3, width: "100%",
      minHeight: size==="lg"?56:40,
    }}><span style={{ fontSize: size==="lg"?20:16 }}>{icon}</span><span>{label}</span></button>
  );

  // Available players for substitution (not in current lineup)
  const getAvailableSubs = (side: "away"|"home") => {
    ...
    const lineupIds = new Set(lineup.map((p:any)=>p.id));
    return data.players.filter((p:any) => p.teamId === teamId && !lineupIds.has(p.id));
  };
  ...
  return (
    <div style={{ background: K.bg, color: K.text, fontFamily: "'Outfit',system-ui,sans-serif", minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      <div className="scorer-grid">
        ...
        <div style={{ gridColumn: "1/-1", background: "#0a0e1a", borderBottom: `2px solid ${K.border}`, padding: "8px 12px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:16}}>
            ...
            <div style={{textAlign:"center"}}><div style={{fontSize:9,fontWeight:700,color:K.muted}}>OUTS</div><div style={{display:"flex",gap:4,marginTop:3}}>{[0,1,2].map(i=><div key={i} style={{width:16,height:16,borderRadius:8,background:i<(game.outs||0)?K.red:K.border}}/>)}</div></div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            ...
            <button onClick={()=>setShowPitcher(true)} style={{padding:"6px 10px",borderRadius:8,background:K.border,border:"none",color:K.dim,fontSize:10,fontWeight:700,cursor:"pointer"}}>🔄 Pitcher</button>
            <button onClick={()=>{if(confirm("¿Finalizar?"))finishGame(plays);}} style={{padding:"6px 10px",borderRadius:8,background:K.red,border:"none",color:"#fff",fontSize:10,fontWeight:700,cursor:"pointer"}}>🏁 FIN</button>
          </div></div>
        ...
        <div style={{background:"#0d1220",borderRight:`1px solid ${K.border}`,overflow:"auto",padding:"8px 6px"}}>
          ...
          {batLineup.map((p:any,i:number)=>{
            const isActive = i === (batIdx % batLineup.length);
            const stats = getPlayerGameStats(p.id);
            const side = isTop ? "away" as const : "home" as const;
            return (
          ...
          <div style={{marginTop:10,padding:8,borderRadius:10,background:K.input,borderTop:`2px solid ${K.blue}33`}}>
            <div style={{fontSize:9,fontWeight:900,color:K.blue,textTransform:"uppercase",marginBottom:4}}>⚾ LANZADOR ({pitchTm?.abbr})</div>
            ...
            :<button onClick={()=>setShowPitcher(true)} style={{...S.btn("ghost"),padding:"4px 10px",fontSize:10,width:"100%"}}>Asignar</button>}
          </div></div>
        ...
        <div style={{background:"#080c16",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:12}}>
          <div style={{textAlign:"center",marginBottom:14}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:10,justifyContent:"center"}}>
              <span style={{fontWeight:900,fontSize:13,color:K.blue}}>{pitcher?.name||"?"}</span>
        ...
        <div style={{background:"#0d1220",borderLeft:`1px solid ${K.border}`,overflow:"auto",padding:8,display:"flex",flexDirection:"column",gap:8}}>
          <div><div style={{fontSize:9,fontWeight:900,color:K.muted,textTransform:"uppercase",marginBottom:4,paddingLeft:4}}>PITCHEOS</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:5}}>
              <BtnAction label="BOLA" icon="🟢" color={K.green} onClick={addBall}/>
              <BtnAction label="STRIKE" icon="🔴" color={K.red} onClick={()=>addStrike("called")}/>
              <BtnAction label="SWING" icon="💨" color="#f97316" onClick={()=>addStrike("swinging")}/>
              <BtnAction label="FOUL" icon="📐" color={K.yellow} onClick={()=>addStrike("foul")}/></div></div>
          <div><div style={{fontSize:9,fontWeight:900,color:K.muted,textTransform:"uppercase",marginBottom:4,paddingLeft:4}}>HITS</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:5}}>
              <BtnAction label="1B" icon="🏏" color={K.accent} onClick={()=>prepareHit("1B")} size="lg"/>
              <BtnAction label="2B" icon="✌️" color="#14b8a6" onClick={()=>prepareHit("2B")} size="lg"/>
              <BtnAction label="3B" icon="🔱" color="#6366f1" onClick={()=>prepareHit("3B")} size="lg"/>
              <BtnAction label="HR" icon="💥" color={K.red} bg={`${K.red}22`} onClick={()=>prepareHit("HR")} size="lg"/></div></div>
          <div><div style={{fontSize:9,fontWeight:900,color:K.muted,textTransform:"uppercase",marginBottom:4,paddingLeft:4}}>OUTS</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:5}}>
              <BtnAction label="K" icon="💨" color="#a78bfa" onClick={()=>registerOut("K")}/>
              <BtnAction label="FLY" icon="🔼" color="#64748b" onClick={()=>registerOut("FLY")}/>
              <BtnAction label="GROUND" icon="⬇️" color="#64748b" onClick={()=>registerOut("GROUND")}/></div></div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:5}}>
            <button onClick={()=>setShowError(true)} style={{padding:"10px 8px",borderRadius:12,border:`2px solid #f97316`,background:"#f9731615",color:"#f97316",fontWeight:900,fontSize:11,cursor:"pointer",textAlign:"center"}}>🫣 ERROR</button>
            <button onClick={()=>setShowComplex(true)} style={{padding:"10px 8px",borderRadius:12,border:`2px solid ${K.purple}44`,background:`${K.purple}15`,color:K.purple,fontWeight:900,fontSize:11,cursor:"pointer",textAlign:"center"}}>⚙️ COMPLEJA</button>
          </div>
        ...
        <div style={{gridColumn:"1/-1",background:"#0a0e1a",borderTop:`2px solid ${K.border}`,padding:"6px 12px",display:"flex",alignItems:"center",gap:10}}>
          ...
          <div style={{flex:1,display:"flex",gap:6,overflow:"auto"}} className="scroll-x">
            {recentPlays.map((p:any,i:number)=>{
              const icons:any={"1B":"🏏","2B":"✌️","3B":"🔱","HR":"💥","BB":"👁","K":"💨","OUT":"❌","FLY":"🔼","GROUND":"⬇️","DP":"✖️","SAC":"🎯","HBP":"😤","E":"🫣","WP":"🤷","SB":"🏃","CS":"🚔"};
              return <div key={i} style={{flexShrink:0,padding:"4px 10px",borderRadius:8,background:K.input,border:`1px solid ${K.border}`,display:"flex",alignItems:"center",gap:4,fontSize:10}}>
                ...
                {p.errorPosition&&<span style={{color:K.red,fontSize:9}}>E{p.errorPosition}</span>}
              </div>;})}
          </div>
          <button onClick={()=>setShowTraditional(!showTraditional)} style={{padding:"8px 14px",borderRadius:10,background:K.border,border:"none",color:K.dim,fontWeight:700,fontSize:10,cursor:"pointer",flexShrink:0}}>📋 Hoja</button>
        </div>
      </div>

      {/* ═══ HIT CONFIRMATION MODAL ═══ */}
      {showConfirm && (
        <Modal title={`Confirmar ${showConfirm.type}`} onClose={()=>setShowConfirm(null)}>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            {/* Runs scored adjuster */}
            <div>
              <label style={S.label}>Carreras que anotan en esta jugada</label>
              <div style={{display:"flex",gap:6,justifyContent:"center"}}>
                {[0,1,2,3,4].map(n=>(
                  <button key={n} onClick={()=>setShowConfirm({...showConfirm,runs:n})}
                    style={{width:44,height:44,borderRadius:12,border:`2px solid ${showConfirm.runs===n?K.accent:K.border}`,background:showConfirm.runs===n?`${K.accent}22`:K.input,color:showConfirm.runs===n?K.accent:K.text,fontWeight:900,fontSize:18,cursor:"pointer"}}>{n}</button>))}
              </div>
              {showConfirm.runs !== showConfirm.suggestedRuns && (
                <div style={{textAlign:"center",marginTop:6,fontSize:10,color:K.yellow}}>⚠️ Sugerido: {showConfirm.suggestedRuns} — Ajustado: {showConfirm.runs}</div>
              )}
            </div>

            {/* Bases preview */}
            <div>
              <label style={S.label}>Bases después de la jugada (toca para ajustar)</label>
              <div style={{display:"flex",gap:12,justifyContent:"center",padding:10}}>
                {["1ra","2da","3ra"].map((l,i)=>(
                  <button key={i} onClick={()=>{const nb=[...showConfirm.newBases];nb[i]=!nb[i];setShowConfirm({...showConfirm,newBases:nb});}}
                    style={{padding:"10px 18px",borderRadius:10,border:`2px solid ${showConfirm.newBases[i]?K.yellow:K.border}`,background:showConfirm.newBases[i]?K.yellow+"22":"transparent",color:showConfirm.newBases[i]?K.yellow:K.muted,fontWeight:700,fontSize:13,cursor:"pointer"}}>
                    {l} {showConfirm.newBases[i]?"●":"○"}
                  </button>))}
              </div>
            </div>

            <button onClick={confirmHit} style={{...S.btn("primary"),width:"100%",padding:14,fontSize:14}}>
              ✅ Confirmar {showConfirm.type} {showConfirm.runs > 0 ? `(+${showConfirm.runs} carreras)` : ""}
            </button>
          </div>
        </Modal>
      )}
      ...
      {/* ═══ ERROR MODAL ═══ */}
      {showError && (
        <Modal title="🫣 Error Defensivo" onClose={()=>setShowError(false)}>
          <p style={{color:K.dim,fontSize:12,marginBottom:12}}>¿Qué posición cometió el error?</p>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
            {POSITIONS_DEF.map(pos=>(
              <button key={pos} onClick={()=>registerError(pos)}
                style={{padding:14,borderRadius:12,border:`2px solid #f9731644`,background:"#f9731615",color:"#f97316",fontWeight:800,fontSize:12,cursor:"pointer",textAlign:"center"}}>
                {pos}
              </button>))}
          </div>
        </Modal>
      ...
      {showComplex && <Modal title="Jugada Compleja" onClose={()=>setShowComplex(false)}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          {[{key:"HBP",label:"Golpeado",icon:"😤",color:"#eab308"},{key:"DP",label:"Doble Play",icon:"✖️✖️",color:"#dc2626"},{key:"WP",label:"Wild Pitch",icon:"🤷",color:"#64748b"},{key:"PB",label:"Passed Ball",icon:"🧤",color:"#64748b"},{key:"BALK",label:"Balk",icon:"🚫",color:"#f43f5e"},{key:"SB",label:"Base Robada",icon:"🏃",color:"#8b5cf6"},{key:"CS",label:"Atrapado Robando",icon:"🚔",color:"#dc2626"},{key:"SAC",label:"Sacrificio",icon:"🎯",color:"#a3a3a3"}].map(a=>(
            <button key={a.key} onClick={()=>registerComplex(a.key)} style={{padding:14,borderRadius:14,border:`2px solid ${a.color}44`,background:`${a.color}15`,color:a.color,fontWeight:800,fontSize:12,cursor:"pointer",textAlign:"center"}}>
              <div style={{fontSize:22,marginBottom:4}}>{a.icon}</div>{a.label}</button>))}
        </div></Modal>}

      {/* ═══ PITCHER MODAL ═══ */}
      {showPitcher && <Modal title={`Lanzador (${pitchTm?.name})`} onClose={()=>setShowPitcher(false)}>
        {pitchLineup.map((p:any)=>{const ps=getPitcherGameStats(p.id);return(
          <button key={p.id} onClick={async()=>{await up({currentPitcher:{id:p.id,name:p.name,number:p.number}});setShowPitcher(false);}}
            ...
            <div style={{flex:1}}><div style={{fontWeight:700,fontSize:14}}>{p.name}</div>
              {ps.pitches>0&&<div style={{fontSize:9,color:K.muted}}>IP:{ps.ip} K:{ps.K} H:{ps.h} BB:{ps.bb} Lanz:{ps.pitches}</div>}</div>
            {pitcher?.id===p.id&&<span style={S.badge(K.accent)}>Actual</span>}
          </button>)})}</Modal>}

      {/* ═══ SUBSTITUTION MODAL ═══ */}
      {showSub && (
        <Modal title="Sustitución" onClose={()=>setShowSub(null)}>
          <div style={{marginBottom:12,padding:10,background:K.input,borderRadius:10}}>
            ...
            <div style={{fontWeight:800,fontSize:14,color:K.red,marginTop:2}}>
              #{(showSub.side==="away"?(game.awayLineup||[]):(game.homeLineup||[]))[showSub.idx]?.number} {(showSub.side==="away"?(game.awayLineup||[]):(game.homeLineup||[]))[showSub.idx]?.name}
            </div>
          </div>
            ...
            <div key={team} style={{marginBottom:16}}>
              ...
              <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:11,minWidth:400}}>
                <thead><tr style={{color:K.muted,borderBottom:`1px solid ${K.border}`}}>
                  <th style={{textAlign:"left",padding:4,fontSize:9}}>JUGADOR</th>
                  {["VB","H","2B","3B","HR","CI","CA","BB","K","BR","AVG"].map(c=><th key={c} style={{textAlign:"center",padding:4,fontSize:9}}>{c}</th>)}
                </tr></thead>
                <tbody>{lineup.map((p:any)=>{const s=getPlayerGameStats(p.id);const avg=s.vb>0?(s.h/s.vb).toFixed(3):".000";return(
                  <tr key={p.id} style={{borderBottom:`1px solid ${K.border}`}}>
                    <td style={{padding:4,fontWeight:700}}>#{p.number} {p.name}</td>
                    <td style={{textAlign:"center",padding:4}}>{s.vb}</td><td style={{textAlign:"center",padding:4,fontWeight:700}}>{s.h}</td>
...
export function WatchGame({ data, id, nav }: any) {
  ...
  const R:any={"1B":{l:"Sencillo",i:"🏏"},"2B":{l:"Doble",i:"✌️"},"3B":{l:"Triple",i:"🔱"},HR:{l:"Jonrón",i:"💥"},BB:{l:"Base/Bolas",i:"👁"},K:{l:"Ponche",i:"💨"},OUT:{l:"Out",i:"❌"},FLY:{l:"Fly Out",i:"🔼"},GROUND:{l:"Rodado",i:"⬇️"},DP:{l:"Doble Play",i:"✖️"},SAC:{l:"Sacrificio",i:"🎯"},HBP:{l:"Golpeado",i:"😤"},E:{l:"Error",i:"🫣"},WP:{l:"Wild Pitch",i:"🤷"},SB:{l:"Base Robada",i:"🏃"},CS:{l:"Atrapado",i:"🚔"},PB:{l:"Passed Ball",i:"🧤"},BALK:{l:"Balk",i:"🚫"}};
      ...
      {recentPlays.length>0&&<div style={{...S.card}}>
        ...
        <div style={{padding:8}}>{recentPlays.map((p:any,i:number)=>{const r=R[p.result]||{l:p.result,i:"⚾"};return(
          <div key={i} style={{display:"flex",alignItems:"center",gap:6,padding:"6px 4px",borderBottom:i<recentPlays.length-1?`1px solid ${K.border}`:"none"}}>
            ...
            {p.errorPosition&&<span style={{fontSize:9,color:K.red}}>E{p.errorPosition}</span>}
